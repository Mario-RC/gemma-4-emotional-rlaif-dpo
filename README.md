...
predictions, generated Gemma2 reference outputs, and analysis results all live
...
- Metric analysis and DPO comparison against the published Gemma2 reference.

## Data Source

Training and prediction datasets are downloaded automatically from Hugging Face:
...
This command downloads the Hub files into the project-local Hugging Face cache
and prepares the local LlamaFactory dataset files:

| Local file | Hugging Face source |
| --- | --- |
| `datasets/sft_demonstration_dataset.json` | `dialogues/train.json`, filtered to `set == sft-demonstration` |
| `datasets/sft_demonstration_dataset_test.json` | `dialogues/test.json`, filtered to `set == sft-demonstration` |
...
https://huggingface.co/mario-rc/gemma-2-9b-it-emotional-rlaif-dpo
```

The reference is not copied from another local project. It is generated by
running prediction with:

- base model: `google/gemma-2-9b-it`
- adapter: `mario-rc/gemma-2-9b-it-emotional-rlaif-dpo`
- test dataset: `ppo_unlabeled_prompts_dataset_test`

Generate and analyze the Gemma2 reference locally:

```bash
scripts/gemma4.py reference --stage all --strict
```

Only DPO is compared against Gemma2 because the uploaded Gemma2 reference is the
DPO adapter. SFT predictions can still be generated for Gemma4, but they are not
used in the Gemma4-vs-Gemma2 comparison.
...
gemma-4/
  ...
  src/gemma4_project/           # analysis/comparison Python modules
  scripts/
...
Generate the Gemma2 DPO reference before running strict comparison:
...
5. Analysis.
6. Gemma4-vs-Gemma2 DPO comparison, if the Gemma2 reference has been generated.
...
| Gemma2 reference predict | `saves/gemma-2-9b-it-emotional-rlaif-dpo/predict/dpo_3ep/generated_predictions.jsonl` exists |
| Gemma2 reference analysis | `saves/gemma-2-9b-it-emotional-rlaif-dpo/emotional_balanced/ppo_unlabeled_prompts_dataset_test_results.json` exists |
...
scripts/gemma4.py compare e2b --run-name dpo_3ep --strict
```
...
saves/gemma-2-9b-it-emotional-rlaif-dpo/predict/dpo_3ep/generated_predictions.jsonl
saves/gemma-2-9b-it-emotional-rlaif-dpo/emotional_balanced/ppo_unlabeled_prompts_dataset_test_results.json
```
...
`datasets/dataset_info.json` and `datasets/source_manifest.json` trackable.
...
```bash
git add README.md LICENSE .gitignore configs datasets/dataset_info.json datasets/source_manifest.json scripts src
